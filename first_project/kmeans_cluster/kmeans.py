...
def kmeans_display(X, label):
    ...
    plt.plot(X0[:, 0], X0[:, 1], 'b^', ms = 4, alpha = 0.8)
    plt.plot(X1[:, 0], X1[:, 1], 'go', ms = 4, alpha = 0.8)
    plt.plot(X2[:, 0], X2[:, 1], 'rs', ms = 4, alpha = 0.8)
    plt.axis('equal')
    plt.show()
...
def kmeans_assign_label(X, centers):
    #cal pairwise distances btw data and centers
    D = cdist(X, centers)
